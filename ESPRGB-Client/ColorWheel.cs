...
namespace Cyotek.Windows.Forms
{
  ...
  public class ColorWheel : Control, IColorEditor
  {
    ...
    protected virtual PointF GetColorLocation(HslColor color)
    {
      ...
      double l = color.L;
      radius = _radius * color.S * Map(l, 1, 0.5, 0, 1);
    ...
    protected virtual void SetColor(Point point)
    {
      ...
      l = Math.Sqrt(Math.Pow(_centerPoint.X - point.X, 2) + Math.Pow(_centerPoint.Y - point.Y, 2));
      if (l > _radius) l = _radius;
      l = Math.Round(MapValue(0, _radius, 1, 0.50f, l), 3);
      distance = Math.Pow(Math.Pow(dx, 2) + Math.Pow(dy, 2), l);
      ...
      newColor = new HslColor(angle, saturation, l);
